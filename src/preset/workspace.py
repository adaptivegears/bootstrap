...
def clone(ws):
    ...
    os.makedirs(os.path.join(ws.workdir, 'inventory'), exist_ok=True)
    with open(os.path.join(ws.workdir, 'inventory', 'hosts'), 'w') as f:
        f.write('localhost')

    os.makedirs(os.path.join(ws.workdir, 'inventory', 'host_vars'), exist_ok=True)
    with open(os.path.join(ws.workdir, 'inventory', 'host_vars', 'localhost.yml'), 'w') as f:
        f.write('---\n')
